...
public class P3cPmdProfileImporter extends ProfileImporter {

    private final RuleFinder ruleFinder;

    public P3cPmdProfileImporter(RuleFinder ruleFinder) {
        super(PmdConstants.P3C_REPOSITORY_KEY, PmdConstants.PLUGIN_NAME);
    ...
    public RulesProfile importProfile(Reader pmdConfigurationFile, ValidationMessages messages) {
        ...
        for (PmdRule pmdRule : pmdRuleset.getPmdRules()) {
            ...
            if (PmdConstants.XPATH_CLASS.equals(ruleClassName)) {
                ...
                if (ruleRef == null) {
                    messages.addWarningText("A PMD p3c rule without 'ref' attribute can't be imported. see '" + ruleClassName + "'");
                } else {
                    Rule rule = ruleFinder.find(RuleQuery.create().withRepositoryKey(PmdConstants.P3C_REPOSITORY_KEY).withConfigKey(ruleRef));
                    if (rule != null) {
                        ...
                        messages.addWarningText("Unable to import unknown PMD p3c rule '" + ruleRef + "'");
                    }
                }
            }
        }
